...
func getObjectConfig() *meta.ManagedObjectMeta {
	...
	return &meta.ManagedObjectMeta {
		...
		ObjectClassName:        "fvRsProv",
		ObjectNamePrefix:       "rsprov",
...
func getObjectSchema() map[string]*schema.Schema {
	...
	return map[string]*schema.Schema{
		...
		"epg_id": &schema.Schema{
			...
			Required:    	true,
			ForceNew: 		true,
			Description: 	"ACI EPG Provider Contract Reln - Containing EPG ID (DN)",
			ValidateFunc: 	validation.StringLenBetween(1, 255),
		},
		"dn": &schema.Schema{
			Type:        	schema.TypeString,
			Computed:    	true,
			Description: 	"ACI EPG Provider Contract Reln - DN",
		},		
		"priority_level": &schema.Schema{
			Type:     schema.TypeString,
			Optional: true,
			Description: "ACI EPG Provider Contract Reln - QoS Priority Level [prio]",
			Default:  "unspecified",
		...
		"contract_name": &schema.Schema{
			...
			Required:    	true,
			ForceNew: 		true,
			Description: 	"ACI EPG Provider Contract Reln - Contract relative name [tnVzBrCPName]",

...
func getObjectConfig() *meta.ManagedObjectMeta {
	...
	return &meta.ManagedObjectMeta {
		...
		ObjectClassName:        "vzProvSubjLbl",
		ObjectNamePrefix:       "provsubjlbl",
...
func getObjectSchema() map[string]*schema.Schema {
	...
	return map[string]*schema.Schema{
		...
		"parent_id": &schema.Schema{
			...
			Required:    		true,
			ForceNew: 			true,
			Description: 		"ACI Provider Subject Label - Containing EPG ID (DN)",
			ValidateFunc: 		validation.StringLenBetween(1, 255),
		},
		"desc": &schema.Schema{
			Type:     			schema.TypeString,
			Optional: 			true,
			Description: 		"ACI Provider Label - Description.",
			ValidateFunc:		validation.StringLenBetween(1, 128),
		},
		"dn": &schema.Schema{
			Type:        		schema.TypeString,
			Computed:    		true,
			Description: 		"ACI Provider Subject Label - DN",
		},	
		"is_complement": &schema.Schema{
			Type:        		schema.TypeString,
			Optional:    		true,
			Description: 		"ACI Provider Subject Label - EPG CoS Value",
			Default: 			"no",
		...
		"name": &schema.Schema{
			...
			Required: 			true,
			ForceNew: 			true,
			Description: 		"ACI Provider Subject Label - Name",
			ValidateFunc:		validation.StringLenBetween(1, 64),
		},
		"name_alias": &schema.Schema{
			Type:        		schema.TypeString,
			Optional:    		true,
			Description: 		"ACI Provider Subject Label - Name Alias.",
			ValidateFunc: 		validation.StringLenBetween(0, 128),
		},
		"ownerkey": &schema.Schema{
			Type:        		schema.TypeString,
			Optional:    		true,
			Description: 		"ACI Provider Subject Label - Arbitary key for enabling clients to own their data for entity correlation.",
			ValidateFunc: 		validation.StringLenBetween(0, 128),
		},
		"ownertag": &schema.Schema{
			Type:        		schema.TypeString,
			Optional:    		true,
			Description: 		"ACI Provider Subject Label - A tag for enabling clients to add their own data. For example, to indicate who created this object.",
			ValidateFunc: 		validation.StringLenBetween(0, 64),
		},
		"tag": &schema.Schema{
			Type:        		schema.TypeString,
			Optional:    		true,
			Description: 		"ACI Provider Subject Label  - Color Tag",
		},	
	}
}